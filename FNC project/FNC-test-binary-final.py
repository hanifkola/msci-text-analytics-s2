...
def main():
    ...
    inputlen = max_len_body + max_len_head
    ...
    model1 = keras.models.load_model("model100-5.model")
    model2 = keras.models.load_model("model100-6.model")
    ...
    for i in tqdm(range(len(headers_test))):
        ...
        if result_no == 1:
            result2 = model2.predict(input_t)
            result_no = np.argmax(result2)
            if result_no == 0:
    ...
    headers_test.to_csv('model100-final1.csv', index=False, encoding='utf-8')
